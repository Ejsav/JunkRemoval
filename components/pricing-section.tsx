      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {pricing.tiers.map((t) => (
            <div
              key={t.name}
              className={`relative rounded-2xl p-5 sm:p-7 border flex flex-col ${
                t.popular ? "bg-foreground text-background border-foreground shadow-xl" : "bg-card border-border card-shadow"
              }`}
            >
              {t.popular && (
                <span className="absolute -top-3 left-5 sm:left-7 text-[10px] whitespace-nowrap font-black uppercase tracking-[0.16em] bg-accent text-accent-foreground px-3 py-1 rounded-full">
                  Most booked
                </span>
              )}
              ...
              <h3 className="text-base sm:text-lg font-black mb-3 sm:mb-4">{t.name}</h3>
              <p className={`text-2xl sm:text-3xl font-black tracking-tight mb-2 sm:mb-3 ${t.popular ? "text-accent" : "text-foreground"}`}>{t.price}</p>
              <p className={`text-[13px] sm:text-sm leading-snug sm:leading-relaxed flex-1 ${t.popular ? "text-background/75" : "text-muted-foreground"}`}>{t.description}</p>
            </div>
          ))}
        </div>
